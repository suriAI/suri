...
def get_attendance_db() -> AttendanceDatabaseManager:
    ...
    if attendance_db is None:
        raise HTTPException(
            status_code=500, detail="Attendance database not initialized"
        )
...
def generate_person_id(
    name: str, db: AttendanceDatabaseManager, group_id: str = None
) -> str:
...
async def create_group(
    ...
    try:
        ...
        if not created_group:
            raise HTTPException(
                status_code=500, detail="Failed to retrieve created group"
            )
...
async def get_group(
    group_id: str, db: AttendanceDatabaseManager = Depends(get_attendance_db)
):
    ...
    try:
        ...
        if not member_data.person_id:
            generated_person_id = generate_person_id(
                name=member_data.name, db=db, group_id=member_data.group_id
            )
        ...
        if not added_member:
            raise HTTPException(
                status_code=500, detail="Failed to retrieve added member"
            )
...
async def get_member(
    person_id: str, db: AttendanceDatabaseManager = Depends(get_attendance_db)
):
...
async def get_sessions(
    ...
    try:
        ...
        if needs_recompute:
            ...
            late_threshold_minutes = group.get("settings", {}).get(
                "late_threshold_minutes", 15
            )
            class_start_time = group.get("settings", {}).get(
                "class_start_time", "08:00"
            )
            late_threshold_enabled = group.get("settings", {}).get(
...
async def process_attendance_event(
    ...
    try:
        ...
        if not success:
            raise HTTPException(
                status_code=500, detail="Failed to add attendance record"
            )
        ...
        late_threshold_minutes = (
            group.get("late_threshold_minutes", 15) if group else 15
        )
        class_start_time = group.get("class_start_time", "08:00") if group else "08:00"
        late_threshold_enabled = (
            group.get("settings", {}).get("late_threshold_enabled", False)
            if group
            else False
        )
        ...
        session_data = {
            "id": (
                existing_session["id"] if existing_session else generate_id()
            ),  # Reuse existing ID if updating
            "person_id": event_data.person_id,
            "group_id": member["group_id"],
            "date": today_str,
...
async def get_settings(db: AttendanceDatabaseManager = Depends(get_attendance_db)):
    ...
    try:
        ...
        if sessions:
            ...
            for session in sessions:
                if session.get("status") == "present" and not session.get(
                    "check_in_time"
                ):
                    needs_recompute = True
                    break
...
async def register_face_for_group_person(group_id: str, person_id: str, request: dict):
...
async def bulk_detect_faces(group_id: str, request: dict):
    """
    ...
    try:
        logger.info(
            f"[BULK-DETECT] Face detector available: {face_detector is not None}"
        )
        ...
        if len(images_data) > 50:
            raise HTTPException(
                status_code=400, detail="Maximum 50 images allowed per request"
            )
...
async def bulk_register_faces(group_id: str, request: dict):
    """
    ...
    try:
        ...
        if len(registrations) > 50:
            raise HTTPException(
                status_code=400, detail="Maximum 50 registrations allowed per request"
            )

...
export class BackendProcessManager {
    ...
    private setupProcessHandlers(): void {
        ...
        this.process.on("exit", (code, signal) => {
            console.log(`[BackendProcessManager] Process exited with code ${code}${signal ? ` and signal ${signal}` : ""}`);
            this.status.isRunning = false;
    ...
    private killAllBackendProcesses(): void {
        ...
        for (let i = 0; i < maxAttempts; i++) {
            try {
                if (isWin) {
                    const checkResult = execSync('tasklist /FI "IMAGENAME eq server.exe" /NH', { encoding: "utf8", timeout: 2000 });
                    if (checkResult.includes("INFO: No tasks") || !checkResult.includes("server.exe")) return;
                    try { execSync("taskkill /F /IM server.exe /T", { stdio: "ignore", timeout: 2000 }); } catch { /* silent */ }
                    try {
                        const pidsOutput = execSync('tasklist /FI "IMAGENAME eq server.exe" /NH /FO CSV', { encoding: "utf8", timeout: 2000 });
                        pidsOutput.split("\n").forEach(line => {
                            if (line.includes("server.exe")) {
                                const match = line.match(/"(\d+)"/);
                                if (match?.[1]) {
                                    try { execSync(`taskkill /F /PID ${match[1]} /T`, { stdio: "ignore", timeout: 1000 }); } catch { /* silent */ }
                                }
                            }
                        });
                    } catch { /* silent */ }
                    ...
                    try {
                        const checkResult = execSync("pgrep -f server", { encoding: "utf8", timeout: 2000 });

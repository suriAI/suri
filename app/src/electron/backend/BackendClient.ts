...
export class BackendClient {
    ...
    async checkReadiness(isRunning: boolean): Promise<{ ready: boolean; modelsLoaded: boolean; error?: string }> {
        try {
            if (!isRunning) return { ready: false, modelsLoaded: false, error: "Backend service not started" };

            const health = await this.checkAvailability();
            if (!health.available) return { ready: false, modelsLoaded: false, error: "Backend health check failed" };
